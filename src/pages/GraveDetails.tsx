...
const GraveDetails = () => {
  ...
  const fetchGrave = async () => {
    try {
      ...
      if (error) {
        console.warn('Supabase error, trying mock data:', error);
        
        // Fallback to mock data
        const mockGrave = mockGraves.find(g => g.id === id);
        if (mockGrave) {
          const mockGraveData: GraveData = {
            id: mockGrave.id,
            title: mockGrave.title,
            epitaph: mockGrave.epitaph,
            category: mockGrave.category,
            created_at: mockGrave.timestamp,
            featured: mockGrave.featured,
            package_type: mockGrave.packageType,
            shares: mockGrave.shares,
            profiles: {
              username: mockGrave.author.toLowerCase().replace(/\s+/g, '_'),
              display_name: mockGrave.author,
            },
            reactions: Object.entries(mockGrave.reactions).flatMap(([type, count]) => 
              Array(count).fill(null).map((_, index) => ({
                id: `mock_${mockGrave.id}_${type}_${index}`,
          ...
          setGrave(mockGraveData);
          
          // Calculate reaction counts for mock data
          const counts = { skull: 0, fire: 0, crying: 0, clown: 0 };
          Object.entries(mockGrave.reactions).forEach(([type, count]) => {
